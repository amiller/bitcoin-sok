...
\begin{itemize}
\item {\em Validation Costs.} The computational resource burden imposed on the network. This primarily consists of a) the total amount of communications that must be broadcast to the network (i.e., the size and number of transactions involved), b) the amount of storage that the transaction places in the replicated index, and c) the cost of validating a transaction.
\item {\em Party Costs.} This includes external storage, computation, and communication, as well as the amount of time and interaction required.
\item {\em Trust Model} Attack model and failure modes (e.g., sensitivity to race conditions). Is trust between the participants required (or utilized)?
...
\hline
\textbf{Protocol} & \textbf{Comms} & \textbf{CPU} & \textbf{Space} & \textbf{Comms} & \textbf{CPU} & \textbf{Space} & \textbf{Time} & \textbf{Trust} & \textbf{Fork Required} & Other \\

\hline Purchase (Full) & 
$\Oh{1}$ & $\Oh{\log m}$ & $\Oh{1}$ & 
$\Oh{1} \overall{\Oh{nc}}$ & $\Oh{\log m} \overall{\Oh{nc \log m}}$ & $\Oh{1} \overall{\Oh{m}}$ & \riskpower & 
Buyer trusts Seller & No & \\

\hline Purchase (SPV) & 
$\Oh{1}$ & $\Oh{\log m}$ & $\Oh{1}$ & 
$\Oh{\log c} \overall{\Oh{n}}$ & $\Oh{\log c} \overall{\Oh{n}}$ & $\Oh{1} \overall{\Oh{1}}$ & \riskpower & 
Buyer trusts Seller & No & \\
...
$\Oh{1}$ & $\Oh{\log m}$ & $\Oh{1}$ & 
$\Oh{n_C + \log c} \overall{\Oh{n}}$ & $\Oh{n_C + \log c} \overall{\Oh{n}}$ & $\Oh{1} \overall{\Oh{1}}$ & \riskpower & 
 & & \\
